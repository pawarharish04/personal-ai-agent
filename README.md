...
A **local-first** desktop assistant powered by Groq (`openai/gpt-oss-20b`). Browses the web, reads and sends email, and manages your Google Calendar — all while keeping every piece of data encrypted on your own machine. No backend, no cloud storage, no analytics.
...
|---|---|---|
| **Chat with Groq Model** | Native conversation | No |
...
- Playwright-scraped page content is always treated as **untrusted data** and clearly framed before being passed to the AI, preventing prompt injection.

---

## Global Push-to-Talk (Voice Input)

This application includes a system-wide "Push-to-Talk" feature using the **Right Alt** key.

- **How it works:** When you hold the Right Alt key, your microphone starts recording. When you release it, the audio is sent to the backend and transcribed into text.
- **Privacy:** The global keyboard hook (using `uiohook-napi`) listens system-wide to detect *only* the Right Alt key. All other keystrokes are completely ignored and dropped; they are never logged or stored.
- **OS Permissions:** Because this requires a global keyboard listener, your OS may prompt you for permissions (e.g., macOS will require you to grant "Accessibility" access to your terminal or the packaged app to allow it to intercept key presses globally).

---

## Project Structure

A quick overview of key files to understand the architecture:
- `main.js` & `preload.js`: Electron process entry points and the secure IPC bridge.
- `renderer/app.js`: The frontend chat UI logic and event listeners.
- `src/orchestrator.js`: The "brain" loop — manages the plan/act/observe cycle and talks to Groq.
- `src/tools/`: Contains all capability modules (Browser, Gmail, Calendar, Approval Gate).
- `src/memory/db.js`: Local SQLite storage implementation.
- `src/memory/memoryManager.js`: The tiered memory management system. It runs in the background to summarize older chat sessions to ensure the active context window sent to Groq remains small and cost-effective.
- `src/memory/backfill-risk-level.js`: A one-time database migration script used to add a risk level column to older, existing audit logs in SQLite.
